...
MODULE_AUTHOR("rahulraghu94@gmail.com");
MODULE_VERSION("1.1");
MODULE_DESCRIPTION("Sample module on usage of reader/write Semaphore");
/* Create kobject and use sysfs to export those objects */

/* Name of the directory entry in sysfs file system */
#define SYSFS_NAME "sysfs_lkm_rwlock"
#define DRV_NAME "sysfs_lkm_rwlock"

/* sysfs sysfs-lkm/id file node read/write buffer */
static const char text[] = "random_condom";
static int len = sizeof(text);

/* sysfs sysfs-lkm/foo file node read/write buffer */
static char foo_kbuff[PAGE_SIZE];

/* Reader/Writer semaphore for foo sysfs file.
 * Allow multiple readers but one writer
 */
...
static struct kobj_attribute id_attribute =
	__ATTR(id, 0660, id_show, id_store);
